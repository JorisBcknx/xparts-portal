...
import ASMCustomerSelection from './ASMCustomerSelection';
import './ASMLogin.css';

function ASMLogin({ isOpen, onClose }) {
  const { startASMSession, showCustomerSelection } = useASM();
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');

  if (!isOpen && !showCustomerSelection) return null;

  const handleSubmit = (e) => {
    e.preventDefault();
    if (startASMSession(email, password)) {
      setEmail('');
      setPassword('');
      // Don't close yet - wait for customer selection
    }
  };

  // Show customer selection modal after login
  if (showCustomerSelection) {
    return <ASMCustomerSelection isOpen={true} onClose={onClose} />;
  }

  return (
    <div className="asm-login-overlay" onClick={onClose}>
      <div className="asm-login-modal" onClick={(e) => e.stopPropagation()}>
        <button className="asm-login-close" onClick={onClose}>
        ...
        <div className="asm-login-header">
          ...
          <h2>Assisted Service Module</h2>
          <p>Log in to emulate customer sessions and manage quotes</p>
        </div>

        <form onSubmit={handleSubmit} className="asm-login-form">
          <div className="asm-form-field">
          ...
          <button type="submit" className="asm-login-submit">
            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="asm-demo-info">
            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2"/>
              <path d="M12 16V12M12 8H12.01" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
            </svg>
            <p>Demo mode: Any email and password combination will be accepted</p>
          </div>
        </form>
      </div>
    </div>
